...
$stmt = "UPDATE `gckeeper`.`paciente` SET nome=:nome, sexo=:sexo, data_de_nascimento=:data_de_nascimento, cpf=:cpf, celular=:celular, cep=:cep, uf=:uf, cidade=:cidade, bairro=:bairro, rua=:rua, numero_de_residencia=:numero_de_residencia, email=:email, id_da_empresa=:id_da_empresa, passou_na_clinica=:passou_na_clinica, tipo_exame=:tipo_exame WHERE id=:id";
...
$dados->bindParam(':id_da_empresa', $id_da_empresa);
$dados->bindParam(':passou_na_clinica', $passou);
$dados->bindParam(':tipo_exame', $tipo_exame);
...
try {
    ...
    echo "<script>alert('Registro alterado com sucesso!'); window.location.href = 'menu_paciente.php?id=$id';</script>";
} catch (Exception $e) {
    $pdo->rollBack();
